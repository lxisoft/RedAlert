...
import com.lxisoft.crimestopper.service.impl.ComplaintServiceImpl;
...
public class LocationResource {
	...
	private static final String ENTITY_NAME = "crimestopperLocation";
	 ...
	 *         locationDTO, or with status 400 (Bad Request) if the location has
	 *         already an ID
	 ...
	 *         locationDTO, or with status 400 (Bad Request) if the locationDTO is
	 *         not valid, or with status 500 (Internal Server Error) if the
	 *         locationDTO couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<LocationDTO> updateLocation(@RequestBody LocationDTO locationDTO) throws URISyntaxException {
		...
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, locationDTO.getId().toString())).body(result);
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and the list of locations in
	 *         body
	 */
	 ...
	 *
	 * @param id
	 *            the id of the locationDTO to retrieve
	...
	@GetMapping("complaints/{complaintId}/location")
	@Timed
	public ResponseEntity<LocationDTO> getLocationOfComplaintId(@PathVariable long complaintId) {
		log.debug("REST request to get  Location of Complaint: {}", complaintId);
		Optional<LocationDTO> locationDTO = locationService.findByComplaintId(complaintId);
		return ResponseUtil.wrapOrNotFound(locationDTO);
	}
